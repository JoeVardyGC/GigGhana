...
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=users&msg=Invalid+request&t=error'); exit; }
$uid    = (int)($_POST['user_id'] ?? 0);
$action = $_POST['action'] ?? '';
$db     = adminGetDB();
$admin  = currentAdmin();
if (!$uid) { header('Location: /gigghana/admin/dashboard.php?panel=users&msg=Invalid+user&t=error'); exit; }
switch ($action) {
    case 'ban':
...
header('Location: /gigghana/admin/dashboard.php?panel=users&msg='.urlencode($msg ?? 'Done').'&t='.($t??'success'));
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=jobs&msg=Invalid&t=error'); exit; }
...
header('Location: /gigghana/admin/dashboard.php?panel=jobs&msg='.urlencode($msg ?? 'Done').'&t=success');
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=disputes&msg=Invalid&t=error'); exit; }
...
if (!isset($statusMap[$action])) { header('Location: /gigghana/admin/dashboard.php?panel=disputes&msg=Invalid+action&t=error'); exit; }
...
header('Location: /gigghana/admin/dashboard.php?panel=disputes&msg='.urlencode($msg).'&t=success');
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=analytics&msg=Invalid&t=error'); exit; }
...
header('Location: /gigghana/admin/dashboard.php?panel=analytics&msg='.urlencode($msg).'&t=success');
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=transactions&msg=Invalid&t=error'); exit; }
...
header('Location: /gigghana/admin/dashboard.php?panel=transactions&msg='.urlencode($msg ?? 'Done').'&t='.($t??'success'));
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=Invalid+request&t=error'); exit; }
$admin   = currentAdmin();
...
header('Location: /gigghana/admin/dashboard.php?panel=settings&msg='.urlencode($msg ?? 'Done').'&t='.($t??'success'));
exit;
endif;
...
require_once __DIR__ . '/../config/admin_auth.php';
adminOnly();
if (!adminCheckCSRF() || $_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=Invalid&t=error'); exit; }
$admin  = currentAdmin();
$db     = adminGetDB();
...
if (!$row || !password_verify($cur, $row['password_hash'])) {
    header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=Current+password+is+wrong&t=error'); exit;
}
if (strlen($new) < 8) {
    header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=Password+must+be+at+least+8+characters&t=error'); exit;
}
if ($new !== $conf) {
    header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=Passwords+do+not+match&t=error'); exit;
}
...
adminLogout(false);
header('Location: /gigghana/admin/login.php?reason=password_changed');
exit;
endif;
...
if (!hash_equals($_SESSION['admin_csrf'] ?? '', $csrfGet)) {
    header('Location: /gigghana/admin/dashboard.php?panel=settings&msg=CSRF+invalid&t=error'); exit;
}
$action = $_GET['action'] ?? '';
$db     = adminGetDB();
$admin  = currentAdmin();
