<?php
/**
 * GigGhana — provider/messages.php  (v3 — 5 bugs fixed)
 *
 * FIXES APPLIED:
...
if (!$activeConvId && isset($_GET['start'])) {
    ...
    if ($otherId && $otherId !== $userId) {
        try {
            ...
            header("Location: " . APP_URL . "/provider/messages.php?conv={$cid}");
            exit;
...
try {
    ...
    $stC = $db->prepare("
        SELECT
            ...
            /* Real online: last_seen within 300 seconds */
    ...
    if ($activeConvId) {
        ...
        if ($activeConv) {
            ...
            /* Other user — include phone + last_seen */
            $stO = $db->prepare("
...
/* Last message id for poll sinceId */
...
<title>Messages — GigGhana Provider</title>
...
.topnav{height:var(--nav-h);padding:0 18px;flex-shrink:0;z-index:300;
  ...
  background:rgba(12,14,20,.92);backdrop-filter:blur(28px);border-bottom:1px solid var(--bd);}
...
.navlink.on{background:rgba(0,212,200,.1);color:var(--teal);}
...
.sni.on{background:rgba(0,212,200,.12);color:var(--teal);}
...
.sbox:focus-within{border-color:var(--teal);box-shadow:0 0 0 3px var(--cyan-glo);}
...
.ftab.on{background:rgba(0,212,200,.1);border-color:rgba(0,212,200,.3);color:var(--teal);}
...
.t-empty a{color:var(--teal);}

/* Thread items */
.titem{display:flex;align-items:center;gap:11px;padding:13px 15px;
...
.titem.on{background:rgba(0,212,200,.07);border-left:3px solid var(--teal);padding-left:12px;}
.titem.on .tiname{color:var(--teal);}
.titem.unread::before{content:'';position:absolute;left:0;top:0;bottom:0;
  width:3px;background:var(--coral);}
.titem.on.unread::before{display:none;}
...
/* Online dot — only rendered when DB says online */
...
.tiname{font-family:var(--fh);font-size:13px;font-weight:800;}
.titime{font-size:10.5px;color:var(--tx-3);white-space:nowrap;flex-shrink:0;}
.tiprof{font-size:11px;color:var(--violet);font-weight:700;font-family:var(--fh);margin-bottom:2px;}
.tiprev{font-size:12px;color:var(--tx-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.tiprev.bold{color:var(--tx-2);font-weight:600;}
.tijob{font-size:10.5px;color:var(--tx-4);margin-top:2px;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.tiunread{background:var(--coral);color:#fff;font-size:10px;font-weight:800;
  min-width:18px;height:18px;border-radius:50px;display:flex;align-items:center;
  justify-content:center;padding:0 5px;font-family:var(--fh);flex-shrink:0;margin-left:2px;}
...
/* Header online dot — controlled by JS via data-online */
...
/* Status label — updated by JS poll */
...
.btn-cyan{background:linear-gradient(135deg,var(--teal),var(--teal));color:#000;font-weight:800;}
...
/* Call button */
.btn-call{background:rgba(31,217,160,.12);border:1px solid rgba(31,217,160,.28);
...
.jbtitle{font-family:var(--fh);font-size:12.5px;font-weight:800;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
.jp-progress{background:rgba(0,212,200,.1);color:var(--teal);border:1px solid rgba(0,212,200,.22);}
...
.bav.mine-av{background:linear-gradient(135deg,var(--teal),var(--teal));}
.bwrap{display:flex;flex-direction:column;gap:1px;max-width:min(66%,460px);}
.brow.mine .bwrap{align-items:flex-end;}
.rsnip{background:rgba(0,0,0,.25);border-left:3px solid var(--teal);
  border-radius:8px 8px 0 0;padding:7px 11px;margin-bottom:2px;font-size:11.5px;color:var(--tx-2);}
.rsnip-name{font-family:var(--fh);font-size:10.5px;font-weight:800;color:var(--teal);margin-bottom:2px;}
...
.bub.sent{background:linear-gradient(135deg,rgba(31,217,160,.2),rgba(31,217,160,.13));
...
.filebub:hover{border-color:var(--teal);background:rgba(0,212,200,.05);}
.fibico{width:36px;height:36px;border-radius:9px;flex-shrink:0;
...
.fi-img{background:rgba(0,212,200,.1);border:1px solid rgba(31,217,160,.2);}
...
.fibdl{color:var(--teal);font-size:16px;flex-shrink:0;}
...
.voice-bub.sent{background:linear-gradient(135deg,rgba(31,217,160,.2),rgba(31,217,160,.13));
  border:1px solid rgba(0,212,200,.23);border-bottom-right-radius:4px;}
.voice-bub.recv{background:var(--s3);border-bottom-left-radius:4px;}
.vn-play{width:34px;height:34px;border-radius:50%;flex-shrink:0;border:none;
  display:flex;align-items:center;justify-content:center;font-size:14px;
  cursor:pointer;transition:var(--e);}
.voice-bub.sent .vn-play{background:rgba(0,212,200,.25);color:var(--teal);}
...
.vn-bar.active{background:var(--teal);}
...
/* Bubble meta — ticks from DB only */
...
.t-s{color:var(--tx-3);}   /* ✓  sent */
.t-d{color:var(--tx-2);}   /* ✓✓ delivered */
.t-r{color:var(--teal);}   /* ✓✓ read */

/* Typing indicator — only shown when poll says is_typing=true */
.typrow{display:flex;align-items:flex-end;gap:7px;margin-top:4px;animation:bIn .2s ease;}
.typbub{background:var(--s3);border-radius:16px;border-bottom-left-radius:4px;
...
@keyframes td{0%,60%,100%{transform:translateY(0);}30%{transform:translateY(-5px);background:var(--teal);}}

/* Empty chat */
.chat-empty{flex:1;display:flex;flex-direction:column;align-items:center;
  justify-content:center;text-align:center;padding:40px 24px;position:relative;z-index:1;}
.ceav{width:70px;height:70px;border-radius:50%;margin:0 auto 16px;
  background:linear-gradient(135deg,var(--coral),var(--violet));
  display:flex;align-items:center;justify-content:center;
  font-family:var(--fh);font-size:24px;font-weight:800;color:#fff;
  overflow:hidden;border:3px solid rgba(0,212,200,.3);}
...
.rb-stripe{width:3px;background:var(--teal);border-radius:2px;align-self:stretch;flex-shrink:0;}
...
.upitem{position:relative;width:54px;height:54px;border-radius:10px;overflow:hidden;
  background:var(--s4);border:1px solid var(--bd);
  display:flex;align-items:center;justify-content:center;font-size:22px;}
.upitem img{width:100%;height:100%;object-fit:cover;}
.upitem-rm{position:absolute;top:-4px;right:-4px;width:17px;height:17px;border-radius:50%;
  background:var(--red);color:#fff;font-size:10px;font-weight:800;
  display:flex;align-items:center;justify-content:center;border:2px solid var(--bg);cursor:pointer;}

/* Voice recording UI */
.rec-bar{display:none;align-items:center;gap:12px;
  background:rgba(255,77,109,.08);border:1px solid rgba(255,77,109,.2);
  border-radius:var(--rs);padding:10px 14px;margin-bottom:10px;}
...
.rec-cancel{background:none;border:none;color:var(--tx-3);font-size:13px;
  font-weight:600;font-family:var(--fh);margin-left:auto;cursor:pointer;transition:var(--e);}
.rec-cancel:hover{color:var(--tx);}

.input-row{display:flex;align-items:flex-end;gap:8px;}
.attgrp{display:flex;gap:5px;flex-shrink:0;}
.attbtn{width:38px;height:38px;border-radius:var(--rs);background:rgba(255,255,255,.04);
  border:1.5px solid var(--bd);display:flex;align-items:center;justify-content:center;
  font-size:16px;color:var(--tx-2);transition:var(--e);position:relative;overflow:hidden;}
.attbtn:hover{background:rgba(0,212,200,.1);border-color:rgba(0,212,200,.3);color:var(--teal);}
.attbtn input[type=file]{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%;font-size:0;}
/* Voice btn — turns red while recording */
.attbtn.recording{background:rgba(255,77,109,.15);border-color:rgba(255,77,109,.4);color:var(--red);}
.msgbox{flex:1;display:flex;align-items:flex-end;background:rgba(0,0,0,.3);
  border:1.5px solid var(--bd);border-radius:13px;padding:0 12px;gap:6px;transition:var(--e);}
.msgbox:focus-within{border-color:var(--teal);box-shadow:0 0 0 3px var(--cyan-glo);}
.msgta{flex:1;background:transparent;border:none;outline:none;color:var(--tx);
  font-family:var(--fb);font-size:14px;resize:none;overflow-y:hidden;
  max-height:120px;padding:10px 0;line-height:1.6;}
...
.sendbtn{width:42px;height:42px;border-radius:13px;flex-shrink:0;
  background:linear-gradient(135deg,var(--coral),#FF9A6C);border:none;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 14px var(--coral-glo);transition:var(--e);}
...
.epick{position:absolute;bottom:calc(100% + 8px);right:18px;background:var(--s2);
  border:1px solid var(--bd2);border-radius:16px;padding:12px;
  box-shadow:0 18px 52px rgba(0,0,0,.65);display:none;z-index:50;width:274px;backdrop-filter:blur(22px);}
...
.no-chat{flex:1;display:flex;flex-direction:column;align-items:center;
  justify-content:center;text-align:center;padding:48px 32px;position:relative;z-index:1;}
.nc-orb{width:96px;height:96px;border-radius:50%;margin:0 auto 22px;
  background:linear-gradient(135deg,rgba(0,212,200,.14),rgba(255,107,74,.08));
  border:2px solid rgba(31,217,160,.2);display:flex;align-items:center;justify-content:center;
  font-size:42px;animation:orb 4s ease-in-out infinite;}
...
/* Lightbox */
.lightbox{display:none;position:fixed;inset:0;z-index:3000;
  background:rgba(0,0,0,.95);backdrop-filter:blur(22px);
  align-items:center;justify-content:center;padding:20px;}
.lightbox.open{display:flex;}
.lightbox img{max-width:90vw;max-height:90vh;object-fit:contain;border-radius:12px;}
.lb-close{position:absolute;top:16px;right:18px;background:rgba(255,255,255,.1);
  border:1px solid rgba(255,255,255,.2);color:#fff;width:36px;height:36px;border-radius:50%;
  display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:17px;transition:var(--e);}
.lb-close:hover{background:rgba(255,77,109,.3);}

/* Context menu */
.ctxmenu{position:fixed;z-index:2000;background:var(--s2);border:1px solid var(--bd2);
  border-radius:12px;padding:6px;min-width:158px;
  box-shadow:0 14px 44px rgba(0,0,0,.65);display:none;backdrop-filter:blur(18px);}
...
.ctxi{display:flex;align-items:center;gap:9px;padding:9px 13px;border-radius:9px;
  font-size:13px;font-weight:600;font-family:var(--fh);cursor:pointer;transition:var(--e);color:var(--tx-2);}
...
.toast{display:flex;align-items:center;gap:11px;background:var(--s2);border:1px solid var(--bd2);
  padding:11px 15px;border-radius:var(--rs);max-width:320px;
  box-shadow:0 14px 40px rgba(0,0,0,.6);animation:tIn .3s ease;backdrop-filter:blur(18px);}
...
<nav class="topnav">
  ...
  <div class="nav-links">
    <a href="<?= APP_URL ?>/provider/dashboard.php" class="navlink">📊 Dashboard</a>
    <a href="<?= APP_URL ?>/provider/proposals.php"   class="navlink">📩 My Bids</a>
    <a href="<?= APP_URL ?>/provider/messages.php"  class="navlink on">💬 Messages</a>
    <a href="<?= APP_URL ?>/jobs.php" class="navlink">🔍 Jobs</a>
  </div>
  <div class="nav-r">
...
<div class="app">
  <!-- ICON SIDEBAR -->
  <nav class="snav">
    <a class="sni" href="<?= APP_URL ?>/provider/dashboard.php"  title="Dashboard">📊</a>
    <a class="sni" href="<?= APP_URL ?>/jobs.php"   title="Post Job">✏️</a>
    <a class="sni" href="<?= APP_URL ?>/provider/proposals.php"    title="My Jobs">📋</a>
    <a class="sni" href="<?= APP_URL ?>/provider/proposals.php"  title="Proposals">📩</a>
    <a class="sni on" href="<?= APP_URL ?>/provider/messages.php" title="Messages">
      💬<?php if($totalUnread>0):?><div class="sni-dot"></div><?php endif;?>
    </a>
    <a class="sni" href="<?= APP_URL ?>/jobs.php"  title="Find Talent">🔍</a>
    ...
    <div class="tlist" id="tlist">
      ...
      <div class="t-empty">
        ...
        <p>No conversations yet.<br>
          <a href="<?= APP_URL ?>/jobs.php">Post a job</a>
      ...
      <?php else:
        foreach($conversations as $c):
          ...
          /* REAL online status from DB */
          $cOnline = (bool)$c['is_online'];
      ?>
      <a href="<?= APP_URL ?>/provider/messages.php?conv=<?= $c['id'] ?>"
  ...
  <div class="chat-panel <?= $activeConvId?'mobopen':'' ?>" id="chatPanel">
    ...
    <div class="chat-head">
      ...
      <div class="chinfo">
        ...
        <div class="chmeta">
          ...
          <!-- Status label updated by JS poll — initial value from DB -->
      ...
      <div class="chacts">
        ...
        <?php endif;?>
        <!-- REAL CALL: opens tel: link with provider's actual phone number -->
        <?php if(!empty($oPhone)):?>
    ...
    <div class="chat-body" id="chatBody">
      ...
      <div class="brow <?= $isMine?'mine':'' ?> <?= $hideAv?'hideav':'' ?>"
        ...
        <div class="bwrap">
          ...
          <!-- TICKS: 100% from DB, no fake simulation -->
          <div class="bmeta">
            <span><?= $mTime ?></span>
            <?php if($isMine):
      ...
      <!-- TYPING INDICATOR — hidden by default, shown ONLY when poll says is_typing=true -->
    ...
    <div class="no-chat">
      ...
      <div class="nc-ctas">
        <a href="<?= APP_URL ?>/jobs.php"  class="btn btn-coral">🔍 Browse Jobs</a>
        <a href="<?= APP_URL ?>/jobs.php" class="btn btn-ghost">My Proposals →</a>
      </div>
    </div>
    <?php endif;?>
  </div>
...
/* ═══════════════════════════════════════════
   ...
   Every 3 seconds: check online + typing state
   Every 30 seconds: send our own heartbeat
═══════════════════════════════════════════ */
let pollTimer = null;
...
function startPolling() {
  ...
  /* Send heartbeat immediately + every 30s */
  sendHeartbeat();
...
function doPoll() {
  fetch(`${APP_URL}/api/presence.php?action=poll&conversation_id=${CONV_ID}&since_id=${LAST_ID}`)
    ...
    .then(d => {
      ...
      /* FIX 1a — Update online dot and status label from real data */
      const dot = document.getElementById('onlineDot');
      const lbl = document.getElementById('statusLbl');
      if (dot) {
        if (d.other_online) { dot.classList.add('live'); }
      ...
      /* FIX 4 — Typing indicator: ONLY show when poll confirms is_typing=true */
      ...
      /* Append new messages received since last poll */
      if (d.new_messages && d.new_messages.length > 0) {
        d.new_messages.forEach(m => {
          if (parseInt(m.id) > LAST_ID) {
            appendPollMsg(m);
            LAST_ID = parseInt(m.id);
          }
        });
        scrollBot();
      }
...
/* FIX 4 — Typing: tell server when WE start/stop typing */
function onTyping() {
  if (!CONV_ID) return;
  if (!isTypingLocal) {
    isTypingLocal = true;
...
/* Append a message received via poll (not optimistic) */
function appendPollMsg(m) {
  ...
  const mTime  = new Date(m.created_at.replace(' ','T')).toLocaleTimeString(
  ...
  if (m.message_type === 'image' && m.file_url) {
    bHTML = `<div class="imgbub ${isMine?'sent':'recv'}" onclick="openLb('${m.file_url}')">
      <img src="${m.file_url}" loading="lazy" class="lazy" onload="this.classList.replace('lazy','loaded')"></div>`;
  } else if (m.file_url) {
    const fn = m.file_name || m.file_url.split('/').pop();
    bHTML = `<a href="${m.file_url}" target="_blank" class="filebub">
      <div class="fibico fi-def">📎</div>
      <div class="fibinfo"><div class="fibname">${esc(fn)}</div></div>
      <div class="fibdl">↓</div></a>`;
  } else {
    bHTML = `<div class="bub ${isMine?'sent':'recv'}">${esc(m.content||'').replace(/\n/g,'<br>')}</div>`;
  }

  /* FIX 5 — Ticks from DB fields only */
  let tick = '';
  if (isMine) {
  ...
  row.innerHTML = `
    <div class="bav ${isMine?'mine-av':''}">${isMine?myAvStr:oAvStr}</div>
    <div class="bwrap">${bHTML}
      <div class="bmeta"><span>${mTime}</span>${tick}</div></div>`;
...
function sendMsg() {
  ...
  fetch(APP_URL + '/api/messages.php', {
    ...
    body: new URLSearchParams({
      action:'send', conversation_id:CONV_ID,
      content:text, message_type:'text',
      reply_to_id:replyId||'', csrf:CSRF,
    })
  })
  .then(r => r.json())
  .then(d => {
    btn.disabled = false;
    if (d.success) {
      /* Update LAST_ID so poll doesn't double-show this message */
      if (d.message_id) {
        LAST_ID = Math.max(LAST_ID, parseInt(d.message_id));
        /* Update data-mid for context menu */
        if (tmpRow) tmpRow.dataset.mid = d.message_id;
      }
      /* FIX 5 — tick comes from server response, not a timeout */
      upgradeTick(tmpRow, 't-d', '✓✓');
    } else {
      toast('Error', d.message || 'Failed to send', 'error');
    }
  })
...
function uploadSend(caption) {
  ...
  Promise.all(proms).then(results => {
    btn.disabled = false;
    if (ta) { ta.value = ''; ag(ta); }
    pending = []; renderStrip(); cancelReply();
    results.forEach(d => {
      if (d.success) {
        addOptimisticBubble(caption, d.message_type, d.file_url, d.file_name, d.file_size);
        if (d.message_id) LAST_ID = Math.max(LAST_ID, parseInt(d.message_id));
      }
    });
    scrollBot();
...
/* Optimistic bubble — returns the row element */
function addOptimisticBubble(content, type, fileUrl, fileName, fileSize) {
  ...
  if (type === 'image' && fileUrl) {
    bHTML = `<div class="imgbub sent" onclick="openLb('${fileUrl}')">
      <img src="${fileUrl}" loading="lazy" class="lazy" onload="this.classList.replace('lazy','loaded')"></div>`;
  } else if (type === 'file' && fileUrl) {
    const fn = fileName || fileUrl.split('/').pop();
    bHTML = `<a href="${fileUrl}" target="_blank" class="filebub">
      <div class="fibico fi-def">📎</div>
      <div class="fibinfo"><div class="fibname">${esc(fn)}</div>
        <div class="fibsize">${fileSize?szFmt(fileSize):'Uploading…'}</div></div>
      <div class="fibdl">↓</div></a>`;
  } else if (type === 'audio' && fileUrl) {
    bHTML = `<div class="voice-bub sent">
      <button class="vn-play" onclick="playVoice(this,'${fileUrl}')">▶</button>
      <div class="vn-waveform">${'<div class="vn-bar" style="height:10px;"></div>'.repeat(18)}</div>
      <span class="vn-dur">Sent</span></div>`;
  } else {
    bHTML = `<div class="bub sent">${esc(content).replace(/\n/g,'<br>')}</div>`;
  }

  /* FIX 5 — start with ✓ sent, server will upgrade via poll */
  row.innerHTML = `
    <div class="bav mine-av">${myAvStr}</div>
    <div class="bwrap">${bHTML}
      <div class="bmeta"><span>now</span><span class="tick t-s" title="Sent">✓</span></div></div>`;
...
/* FIX 5 — upgrade tick on a specific row (no global setTimeout) */
...
async function startRec() {
  ...
  try {
    ...
    recInterval = setInterval(() => {
      ...
      if (recSeconds >= 120) stopRec(); /* max 2 min */
    }, 1000);

    /* Stop when mouse/touch is released */
    document.addEventListener('mouseup', stopRec, { once: true });
    document.addEventListener('touchend', stopRec, { once: true });

  } catch (err) {
    toast('Microphone', 'Could not access microphone. Please allow permission.', 'error');
  }
}
...
function uploadVoiceNote(blob) {
  ...
  fetch(APP_URL + '/api/messages.php', { method:'POST', body:fd })
    ...
    .then(d => {
      btn.disabled = false;
      if (d.success) {
        addOptimisticBubble('', 'audio', d.file_url);
        if (d.message_id) LAST_ID = Math.max(LAST_ID, parseInt(d.message_id));
        scrollBot();
      } else {
        toast('Error', d.message || 'Voice note failed', 'error');
      }
    })
    .catch(() => { btn.disabled = false; toast('Error','Upload failed.','error'); });
}

/* Play voice note in browser */
...
/* ═══════════════════════════════════════════
   EMOJI, FILE QUEUE, TEXTAREA, SCROLL, etc.
═══════════════════════════════════════════ */
const EM = [
  '😊','😂','🙏','👍','👌','🔥','💪','✅','❤️','😍','🥰','🎉',
  '😅','🤔','😮','😁','💯','🌟','⭐','🎊','🙌','👏','🤝','💼',
...
(()=>{
  const g=document.getElementById('epgrid');if(!g)return;
  EM.forEach(e=>{ const b=document.createElement('button');b.className='epbtn';b.textContent=e;b.onclick=()=>insEmoji(e);g.appendChild(b); });
})();
function toggleEp(e){e.stopPropagation();document.getElementById('epick').classList.toggle('open');}
function insEmoji(em){
  const ta=document.getElementById('msgta');if(!ta)return;
...
function queueF(inp,type){
  Array.from(inp.files).slice(0,5-pending.length).forEach(f=>pending.push({file:f,type}));
  renderStrip();inp.value='';
}
function renderStrip(){
  ...
  pending.forEach((p,i)=>{
    ...
    const rm=document.createElement('div');rm.className='upitem-rm';rm.textContent='×';
    rm.onclick=()=>{pending.splice(i,1);renderStrip();};d.appendChild(rm);s.appendChild(d);
  });
}
...
function filterConvs(mode,btn){
  ...
  document.querySelectorAll('.titem').forEach(el=>{
    el.style.display=(mode==='all'?true:mode==='unread'?el.dataset.unread==='1':!!el.dataset.job)?'':'none';
  });
}

/* Mobile */
function convClick(e,id){if(window.innerWidth<=900){e.preventDefault();window.location.href=APP_URL+'/provider/messages.php?conv='+id;}}
function goBack(){document.getElementById('chatPanel').classList.remove('mobopen');history.pushState(null,'',APP_URL+'/provider/messages.php');}
